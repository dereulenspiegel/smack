...
public class PrivacyTest extends SmackTestCase {
	...
	public void testCreateActiveList() {
		try {
			...
			List privacyItems = privacyManager.getPrivacyList(listName)
					.getItems();
	...
	public void testPrivacyItems() {
		try {
			...
			item = new PrivacyItem(PrivacyItem.Type.subscription.name(), true,
					i);
			item.setValue(PrivacyRule.SUBSCRIPTION_BOTH);
			originalPrivacyItems[i] = item;
			i = i + 1;

			item = new PrivacyItem(PrivacyItem.Type.subscription.name(), false,
					i);
			...
			for (int j = 0; j < originalPrivacyItems.length; j++) {
				...
				while ((index < originalPrivacyItems.length)
						&& (originalPrivacyItems[index].getOrder() != receivedItem
								.getOrder())) {
					index++;
				}
				...
				assertEquals(originalItem.isFilterEverything(),
						receivedItem.isFilterEverything());
				assertEquals(originalItem.isFilterIQ(),
						receivedItem.isFilterIQ());
				assertEquals(originalItem.isFilterMessage(),
						receivedItem.isFilterMessage());
				assertEquals(originalItem.isFilterPresence_in(),
						receivedItem.isFilterPresence_in());
				assertEquals(originalItem.isFilterPresence_out(),
						receivedItem.isFilterPresence_out());
			}
	...
	public class PrivacyClient implements PrivacyListListener {
		...
		/**
		 * holds a privacy to hold server requests Clients should not use
		 * Privacy class since it is private for the smack framework.
		 */
